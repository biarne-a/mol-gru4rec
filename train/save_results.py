...
def save_history(all_epoch_metrics: list[dict[str, float]], config: Config):
    print(f"Saving history: {all_epoch_metrics}")
    output_file = f"{config.results_dir}/history_training.p"
    pickle.dump(all_epoch_metrics, open(output_file, "wb"))


def save_predictions(config: Config, data: Data, model, k: int = 10):
    pass
